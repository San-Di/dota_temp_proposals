...
class DotaProposalModel(nn.Module):
    def __init__(self, d_model, d_hidden, n_layers, n_heads, vocab,
        ...
        super(DotaProposalModel, self).__init__()
        ...
        self.rgb_emb = nn.Linear(4096, d_model // 2)
        ...
        '''
            d_model : spatial dimension 1024
        '''
        ...
        nn.Conv1d(in_channels= d_model, out_channels= d_model, kernel_size=(3,3), stride=2, 
    ...
    def forward(self, x, s_pos, s_neg, anno_class,
                sample_prob=0, stride_factor=10, scst=False,
                gated_mask=False):
        
        batch_size, temporal_dim, _ = x.size()
        print("X size B > {} \t T > {}".format(batch_size, temporal_dim))
        dtype = x.data.type()
        ...
        prop_lst = []
        # file = open(ROOT_DIR + '/exp/anc_model.txt', 'w')
        ...
        for i, kernel in enumerate(self.prop_out):
            ...
            if kernel_size <= vis_feat.size(-1):
                ...
                # print("====== Proposal after TCN ==== ", pred_o.shape, kernel)
                anchor_c = Variable(torch.FloatTensor(np.arange(
                    float(kernel_size)/2.0,
                    float(temporal_dim+1-kernel_size/2.0),
                ...
                print("Anchor > center: {} \t\t length: {}".format(anchor_c.shape, anchor_l.shape))
                
                pred_final = torch.cat((pred_o, anchor_l, anchor_c), 1)
                prop_lst.append(pred_final)
                # print(" pred_o + anchors > pred_final : {} ".format(pred_final.shape))
            else:
        ...
        # print(" total predict list ", np.array(prop_lst).shape)
        prop_all = torch.cat(prop_lst, 2)
        # print("Total proposal list > ", prop_all.shape)
        if batch_size != s_pos.size(0) or batch_size != s_neg.size(0):
        ...
        # B x T x H
        batch_mask = Variable(torch.FloatTensor(np.zeros((batch_size,temporal_dim,1))).type(dtype))
        ...
        pe_locs = Variable(torch.zeros(batch_size*4).type(dtype))
        anchor_window_mask = Variable(torch.zeros(batch_size, temporal_dim).type(dtype),
                                      requires_grad=False)
        pred_bin_window_mask = Variable(torch.zeros(batch_size, temporal_dim).type(dtype),
                                      requires_grad=False)
        ...
        # print("******** pred > len : {} \t\t center : {} *************** ".format(pred_len.shape, pred_cen.shape))
        ...
        for b in range(batch_size):
            ...
            for i in range(sample_each):
                ...
                if i == pred_index: # only need once, since one sample corresponds to one sentence only
                    ...
                    gt_window_mask = torch.zeros(temporal_dim, 1).type(dtype)

                    gt_window_mask[
                    max(0, math.floor(gt_cen - gt_len / 2.)):
                    min(temporal_dim, math.ceil(gt_cen + gt_len / 2.)), :] = 1.
                    ...
                    math.floor(max(0, min(temporal_dim-1, pred_start_w.data.item()))):
                    math.ceil(max(1, min(temporal_dim, pred_end_w.data.item())))] = 1.

                    if self.learn_mask:
                        anchor_window_mask[b,
                        max(0, math.floor(anc_cen - anc_len / 2.)):
                        min(temporal_dim, math.ceil(anc_cen + anc_len / 2.))] = 1.

                        pe_locs[b] = pred_start_w
                        pe_locs[batch_size + b] = pred_end_w
                        pe_locs[batch_size*2 + b] = Variable(torch.Tensor([max(0, math.floor(anc_cen - anc_len / 2.))]).type(dtype))
                        pe_locs[batch_size*3 + b] = Variable(torch.Tensor([min(temporal_dim, math.ceil(anc_cen + anc_len / 2.))]).type(dtype))
        ...
        if self.learn_mask:
            ...
            pred_mask = self.mask_model(in_pred_mask).view(batch_size, temporal_dim, 1)

            if gated_mask:
                gate_scores = F.sigmoid(gate_scores)
                window_mask = (gate_scores * pred_bin_window_mask.view(batch_size, temporal_dim, 1)
                # window_mask = (gate_scores * batch_mask
                                +  (1-gate_scores) * pred_mask)
            else:
                window_mask = pred_mask

            mask_loss = F.binary_cross_entropy_with_logits(pred_mask, pred_bin_window_mask.view(batch_size, temporal_dim, 1))
            # mask_loss = F.binary_cross_entropy_with_logits(window_mask, batch_mask)
        else:
            window_mask = pred_bin_window_mask.view(batch_size, temporal_dim, 1)
            # window_mask = batch_mask

        pred_sentence, gt_cent= self.cap_model(x, anno_class, window_mask,
                                               sample_prob=sample_prob)

        scst_loss = None
        if scst:
            scst_loss = self.cap_model.scst(x, batch_mask, anno_class)
        # file.close()
